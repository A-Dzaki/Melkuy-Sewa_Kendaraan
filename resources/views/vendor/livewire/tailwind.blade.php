    ...
    @if ($paginator->hasPages())
        <nav role="navigation" aria-label="Pagination Navigation" class="flex items-center justify-between">
            ...
            <div class="flex flex-col sm:flex-row flex-1 items-center justify-between gap-4">
                ...
                <div class="hidden sm:flex overflow-x-auto max-w-full w-full sm:w-auto pb-2 sm:pb-0 justify-center sm:justify-end">
            ...
            {{-- Custom Mobile Pagination (Compact) --}}
            <div class="flex sm:hidden overflow-x-auto max-w-full w-full pb-2 justify-center mt-4">
                <span class="relative z-0 inline-flex rtl:flex-row-reverse rounded-md shadow-sm">
                    {{-- Previous Arrow --}}
                    @if ($paginator->onFirstPage())
                        <span aria-disabled="true" aria-label="{{ __('pagination.previous') }}">
                    ...
                    @php
                        ...
                        $mobilePages = [];
                        if ($lp <= 4) {
                            $mobilePages = range(1, $lp);
                        } else {
                            if ($cp <= 2) {
                                $mobilePages = [1, 2, 3, '...', $lp];
                            } elseif ($cp >= $lp - 1) {
                                $mobilePages = [1, '...', $lp - 2, $lp - 1, $lp];
                            } else {
                                $mobilePages = [1, '...', $cp - 1, $cp, $cp + 1, '...', $lp];
                            }
                        }
                    @endphp
                    ...
                    @foreach($mobilePages as $page)
                        @if(is_string($page))
                            <span aria-disabled="true"><span class="relative inline-flex items-center px-3 py-2 -ml-px text-sm font-medium text-gray-700 bg-white border border-gray-300 cursor-default leading-5 dark:bg-gray-800 dark:border-gray-600 dark:text-gray-300">...</span></span>
                        @elseif($page == $cp)
                            <span aria-current="page"><span class="relative inline-flex items-center px-3 py-2 -ml-px text-sm font-medium text-gray-500 bg-white border border-gray-300 cursor-default leading-5 dark:bg-gray-800 dark:border-gray-600">{{ $page }}</span></span>
                        @else
                            <button type="button" wire:click="gotoPage({{ $page }}, '{{ $paginator->getPageName() }}')" x-on:click="{{ $scrollIntoViewJsSnippet }}" class="relative inline-flex items-center px-3 py-2 -ml-px text-sm font-medium text-gray-700 bg-white border border-gray-300 leading-5 hover:text-gray-500 focus:z-10 focus:outline-none focus:border-blue-300 focus:ring ring-blue-300 active:bg-gray-100 active:text-gray-700 transition ease-in-out duration-150 dark:bg-gray-800 dark:border-gray-600 dark:text-gray-400 dark:hover:text-gray-300 dark:active:bg-gray-700 dark:focus:border-blue-800">{{ $page }}</button>
                        @endif
                    @endforeach
