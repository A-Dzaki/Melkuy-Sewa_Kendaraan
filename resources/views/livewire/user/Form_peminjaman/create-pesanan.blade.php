...
<div class="bg-slate-50 min-h-screen py-12">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <nav class="flex items-center gap-2 text-sm mb-8">
            <a href="{{ route('user.detail', $kendaraan->id) }}" wire:navigate class="text-slate-500 hover:text-indigo-600 transition-colors">Kembali ke Detail Kendaraan</a>
                ...
                <div class="bg-white rounded-3xl shadow-sm border border-slate-200 overflow-hidden">
                    ...
                    <form wire:submit="save" class="p-6 sm:p-8 space-y-6">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                            {{-- Nama --}}
                            <div>
                                <label for="nama" class="block text-sm font-semibold text-slate-700 mb-2">Nama Lengkap Sesuai KTP <span class="text-rose-500">*</span></label>
                                ...
                                @error('nama') <span class="text-xs text-rose-500 font-semibold mt-1 block">{{ $message }}</span> @enderror
                            </div>

                            {{-- NIK --}}
                            <div>
                                <label for="nik" class="block text-sm font-semibold text-slate-700 mb-2">Nomor Induk Kependudukan (NIK) <span class="text-rose-500">*</span></label>
                            ...
                            <div>
                                ...
                                <input type="email" id="email" wire:model="email" placeholder="Cth: budi@email.com"
                            ...
                            {{-- No HP --}}
                            <div>
                                <label for="no_hp" class="block text-sm font-semibold text-slate-700 mb-2">Nomor WhatsApp <span class="text-rose-500">*</span></label>
                                ...
                                @error('no_hp') <span class="text-xs text-rose-500 font-semibold mt-1 block">{{ $message }}</span> @enderror
                            </div>
                        </div>

                        {{-- Alamat --}}
                        <div>
                            <label for="alamat" class="block text-sm font-semibold text-slate-700 mb-2">Alamat Domisili <span class="text-rose-500">*</span></label>
                            ...
                            @error('alamat') <span class="text-xs text-rose-500 font-semibold mt-1 block">{{ $message }}</span> @enderror
                        </div>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                            {{-- Tanggal Pinjam --}}
                            <div>
                                <label for="tanggal_pinjam" class="block text-sm font-semibold text-slate-700 mb-2">Tanggal Mulai <span class="text-rose-500">*</span></label>
                                <input type="date" id="tanggal_pinjam" wire:model.live="tanggal_pinjam" min="{{ date('Y-m-d') }}"
                                ...
                                @error('tanggal_pinjam') <span class="text-xs text-rose-500 font-semibold mt-1 block">{{ $message }}</span> @enderror
                            </div>

                            {{-- Tanggal Kembali --}}
                            <div>
                                <label for="tanggal_kembali" class="block text-sm font-semibold text-slate-700 mb-2">Tanggal Selesai <span class="text-rose-500">*</span></label>
                                <input type="date" id="tanggal_kembali" wire:model.live="tanggal_kembali" min="{{ $tanggal_pinjam ?: date('Y-m-d') }}"
                                ...
                                @error('tanggal_kembali') <span class="text-xs text-rose-500 font-semibold mt-1 block">{{ $message }}</span> @enderror
                            </div>
                        </div>
                        ...
                        <div class="pt-6">
                            <button type="submit" 
                                ...
                                <span wire:loading.remove wire:target="save">Lanjutkan Pembayaran</span>
                ...
                <div class="bg-slate-900 rounded-3xl shadow-xl overflow-hidden sticky top-24 text-white">
                    ...
                    <div class="p-6">
                        ...
                        <div class="flex items-center gap-4 mb-6">
                            <div class="w-20 h-20 rounded-xl bg-slate-800 overflow-hidden shrink-0 border border-slate-700">
                            ...
                            <div>
                                ...
                                <p class="text-sm text-slate-400 mt-1">{{ $kendaraan->merk }} • {{ $kendaraan->tahun }}</p>
                            </div>
                        </div>
                        ...
                        <div class="space-y-4 text-sm border-t border-slate-800 pt-6">
                            <div class="flex justify-between">
                                ...
                                <span class="font-semibold">Rp {{ number_format($kendaraan->harga_sewa, 0, ',', '.') }}</span>
                            </div>
                            <div class="flex justify-between">
                    ...
                    <div class="p-4 bg-indigo-600/10 border-t border-indigo-500/20">
                        <div class="flex items-start gap-3">
                            ...
                            <p class="text-xs text-indigo-200/80 leading-relaxed">
                                Pastikan data yang Anda masukkan benar. E-tiket / Bukti Pesanan akan dikirimkan ke Email dan Nomor WhatsApp Anda.
                            </p>
                        </div>
                    </div>
                </div>
