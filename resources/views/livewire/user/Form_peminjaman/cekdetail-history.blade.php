...
<div class="bg-slate-50 min-h-screen py-12">
    <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="bg-white rounded-3xl shadow-xl border border-slate-200 overflow-hidden relative">
            {{-- Ticket Header --}}
            <div class="bg-indigo-600 p-8 text-white flex flex-col sm:flex-row justify-between items-center gap-6">
                <div>
                    <p class="text-indigo-200 text-sm font-medium uppercase tracking-wider mb-1">Kode Booking</p>
                    <h2 class="text-4xl font-black font-mono tracking-widest">{{ $peminjaman->kode_booking }}</h2>
                </div>
                ...
                @php
                    $statusConfig = match($peminjaman->status) {
                        ...
                        'berjalan' => ['label' => 'Berjalan', 'bg' => 'bg-indigo-100', 'text' => 'text-indigo-700', 'icon' => 'M13 10V3L4 14h7v7l9-11h-7z'],
                        'selesai' => ['label' => 'Selesai', 'bg' => 'bg-slate-100', 'text' => 'text-slate-700', 'icon' => 'M5 13l4 4L19 7'],
                        'dibatalkan' => ['label' => 'Dibatalkan', 'bg' => 'bg-rose-100', 'text' => 'text-rose-700', 'icon' => 'M6 18L18 6M6 6l12 12'],
                        default => ['label' => 'Unknown', 'bg' => 'bg-slate-100', 'text' => 'text-slate-700', 'icon' => ''],
                    };
                @endphp
                
                <div class="px-4 py-2 rounded-xl {{ $statusConfig['bg'] }} {{ $statusConfig['text'] }} flex items-center gap-2 font-bold shadow-sm">
            ...
            {{-- Decorative circles --}}
            <div class="absolute w-8 h-8 bg-slate-50 rounded-full -left-4 top-[110px] shadow-inner border-r border-slate-200"></div>
            <div class="absolute w-8 h-8 bg-slate-50 rounded-full -right-4 top-[110px] shadow-inner border-l border-slate-200"></div>
            ...
            <div class="p-8 pt-12">
                ...
                <div class="flex items-center gap-6 mb-10">
                    <div class="w-24 h-24 rounded-2xl bg-slate-100 overflow-hidden shrink-0 border border-slate-200 shadow-sm">
                        <img src="{{ $peminjaman->kendaraan->thumbnail_url }}" alt="{{ $peminjaman->kendaraan->nama }}" class="w-full h-full object-cover">
                    </div>
                    <div>
                        <span class="text-xs font-bold uppercase tracking-widest text-indigo-600 mb-1 block">{{ $peminjaman->kendaraan->jenis }}</span>
                        <h3 class="text-2xl font-black text-slate-900">{{ $peminjaman->kendaraan->nama }}</h3>
                        <p class="text-slate-500 font-medium">{{ $peminjaman->kendaraan->merk }} • {{ $peminjaman->kendaraan->tahun }}</p>
                    </div>
                </div>
                ...
                {{-- Price --}}
                <div class="p-6 rounded-2xl bg-indigo-50 border border-indigo-100 flex flex-col md:flex-row justify-between items-center gap-4">
                    <div>
                    ...
                    <p class="text-3xl font-black text-indigo-700">Rp {{ number_format($peminjaman->total_harga, 0, ',', '.') }}</p>
                </div>

                <div class="mt-10 flex flex-col sm:flex-row gap-4 justify-center">
                    <button onclick="window.print()" class="px-6 py-3 rounded-xl bg-white border-2 border-slate-200 text-slate-700 font-bold hover:bg-slate-50 hover:border-slate-300 transition-colors flex items-center justify-center gap-2">
                        <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                    ...
                    @else
                        <a href="{{ route('home') }}" wire:navigate class="px-6 py-3 rounded-xl bg-indigo-600 text-white font-bold hover:bg-indigo-500 shadow-lg shadow-indigo-600/30 transition-all text-center">
                            Kembali ke Beranda
                        </a>
                    @endif
                </div>
                ...
                <div class="mt-8 text-center">
                    <p class="text-sm text-slate-400">Harap simpan kode booking ini. Gunakan saat mengambil kendaraan.</p>
                </div>
            </div>
        </div>
    </div>
...
<style>
    @media print {
        ...
        nav, footer, .bg-slate-50 { background: white !important; }
        button, a[wire\:navigate] { display: none !important; }
